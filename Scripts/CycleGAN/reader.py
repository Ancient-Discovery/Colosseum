...
class Reader():
  def __init__(self, tfrecords_file, image_size=80,
    min_queue_examples=1000, batch_size=4, num_threads=8, name=''):
    """
    Args:
  ...
  def feed(self):
    ...
    with tf.name_scope(self.name):
      ...
      fileNames=features['image/file_name']
      ...
      images = tf.train.shuffle_batch(
            [self._preprocess(image)], batch_size=self.batch_size, num_threads=self.num_threads,
            capacity=self.min_queue_examples + self.batch_size,
