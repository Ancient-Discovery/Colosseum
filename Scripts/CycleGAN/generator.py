  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      ...
      u64 = ops.uk(res_output, 2*self.ngf, is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='u64')    #(4,40,40,128)                             # (?, w/2, h/2, 64)
      u32 = ops.uk(u64, self.ngf, is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='u32', output_size=self.image_size)     #(4,80,80,64)    # (?, w, h, 32)
      ...
      output = ops.c7s1_k(u32, 1, norm=None,
          activation='tanh', reuse=self.reuse, name='output')  #(4,80,80,3)         # (?, w, h, 3)
    # set reuse=True for next call
    self.reuse = True
    self.variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
    # print("G_out") #(4,80,80,3)
    # print(output)
    return output
